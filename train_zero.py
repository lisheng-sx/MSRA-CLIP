...
# 用来保存训练以及验证过程中信息
results_file = "resultszero{}.txt".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
...
def main():
    ...
    for epoch in range(args.epoch):
        ...
        loss_list = []
        idx = 0
        for (image, image_label, mask, seg_idx) in train_loader:
            if idx % (len(train_loader) // 5) == 0:
                ...
                if score >= save_score:
                    ...
                    torch.save({'seg_adapters': model.seg_adapters.state_dict(),
                                ...
                                ckp_path)
                    with open(results_file, 'a') as f:
                        f.write(f"best epoch found: epoch {epoch} batch {idx}\n")
                    print(f'best epoch found: epoch {epoch} batch {idx}')
                print('\n')
            idx += 1
            image = image.squeeze(0).to(device)
            seg_idx = seg_idx.item()#将seg_idx转换为Python整数
...
def test(args, seg_model, test_loader, text_features):
    ...
    with open(results_file, 'a') as f:
        ...
        if CLASS_INDEX[args.obj] > 0:
            f.write(f"Epoch pAUC: {round(roc_auc_score(gt_mask_list.flatten(), segment_scores.flatten()), 4)}\n")
